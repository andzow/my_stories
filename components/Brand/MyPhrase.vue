				...
				const observer = new MutationObserver(mutationsList => {
					for (let mutation of mutationsList) {
						if (mutation.type === 'childList') {
							...
							const spanNoWrap = currentDes.querySelector('.phrase__nowrap')
							if (spanNoWrap) {
								spanNoWrap.style.whiteSpace = 'nowrap'
							}
			...
			this.scrollTriggerVarieble = ScrollTrigger.create({
				...
				onEnter: () => {
					new Typed(currentDes, {
						strings: [
							`Наша миссия – чтобы каждый из Вас стал членом нашей семьи <span class="phrase__nowrap">My stories<span class="phrase__span">…</span></span>^300 через заботу о своих близких и окружающей среде`,
						],
						typeSpeed: 20,
						cursorChar: '',
					})
